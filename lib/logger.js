...
module.exports = {
  ...
  log: function(topic, subject /*args ...*/) {
    ...
    var args = lang.arr.from(arguments),
        topic = args.shift(),
        subject = args.shift(),
        name = subject.id || opts.name || "?",
        status = (subject.status && subject.status())
              || (subject.getState().connection && subject.getState().connection.status && subject.getState().connection.status())
              || subject.getState().connectionState,
        prefix = lang.string.format("[%s %s (%s)] ", topic, name, status),
